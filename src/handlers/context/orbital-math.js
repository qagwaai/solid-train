...
function computeRelativeOrbitPositionKm(ctx, orbit, timestamp) {
  ...
  const meanAnomaly = normalizeAngleRadians(
    ctx,
    meanAnomalyAtEpoch + meanMotionRadPerSec * elapsedSec
  );
  ...
  return rotatePerifocalVector(ctx, { x: xPerifocal, y: yPerifocal, z: 0 }, orbit);
}
...
async function resolveMarketPositionKmAsync(ctx, market, timestamp) {
  ...
  if (orbit && orbit.anchorBodyId) {
    ...
    const fallbackAnchorPosition = FALLBACK_ANCHOR_POSITION_KM[orbit.anchorBodyId] || {
      x: 0,
      y: 0,
      z: 0,
    };
