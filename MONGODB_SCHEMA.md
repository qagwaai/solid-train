...
## SW-15 Bust Persistence Model (M0 Baseline)

**Feature:** SW-15 Minimal Character Bust Builder v0
**Contract baseline:** sw-15-m0-v1
**Entity ownership:** Character-scoped. Bust records belong to playable-character or NPC records, not to the player/account.

### Character Bust Subdocument

Embedded under Player.characters[].bust as an optional subdocument.

A character with no bust record has ust: null or the field absent. Bust is created on first save via character-bust-create and updated in-place thereafter.

#### Fields

- schemaVersion: String (required)
  - Hard-pinned to sw-15-m0-v1 for this contract baseline.
  - Any stored record with an unknown schemaVersion must be treated as a migration target before read/write.
...
- All field values are stored as their canonical enum string form. No aliases or alternative spellings are stored.
- Writes are validated against the full enum set before persistence. Invalid values are rejected; no silent normalization is applied.
- On read, the stored descriptor is returned as-is. No field is synthesized or defaulted at read time.

#### Character Bust Subdocument Shape
...
- deterministicSeed: String (required)
  ...
  - Must be present on all NPC bust records.
- schemaVersion: String (required)
  - Hard-pinned to sw-15-m0-v1 for this contract baseline.
- presetVersion: String (required)
...
### presetVersion and schemaVersion Migration Notes
