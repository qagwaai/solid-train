...
handlerFiles.forEach((file) => {
  ...
  if (content.includes('hasValidSessionAsync') && content.includes('INVALID_SESSION')) {
    console.error(
      `❌ ${path.basename(file)}: Found per-handler session guard (should use registry)`
    );
    errorCount++;
  }
});
...
handlerFiles.forEach((file) => {
  ...
  if (
    !content.includes('handler-utils') &&
...
if (marketBuySell.length > 1) {
  ...
  console.log(
    `ℹ️  ${marketBuySell.length} market buy/sell handlers (should use market-transaction-utils)`
  );
}
